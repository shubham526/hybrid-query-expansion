...
def main():
    ...
    try:
        ...
        bm25_scorer = None
        if BM25_AVAILABLE and args.index_path:
            logger.info("Initializing BM25 scorer...")
            if args.lucene_path:
                setup_lucene(args.lucene_path)
            bm25_scorer = TokenBM25Scorer(args.index_path)
            logger.info("BM25 scorer initialized successfully")
        elif args.index_path:
            logger.warning("BM25 index path provided but scorer not available")
